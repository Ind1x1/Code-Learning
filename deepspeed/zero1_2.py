...
class DeepSpeedZeroOptimizer(ZeROOptimizer):
    ...
    def __init__(self,
        ...
        # vertin checkpoint
        self.vertin_parallel_partitioned_bit16_groups = []

        # a single 32-bit partition of the parallel partitioned parameters
        # that this process will update
        self.single_partition_of_fp32_groups = []
        self.vertin_single_partition_of_fp32_groups =[]
        ...
        self.vertin_params_in_partition_checkpoint = []
        ...
        #vertin optimizer
        self.vertin_param_group = copy.deepcopy(self.optimizer.param_groups)
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            # vertin checkpoint part.
            checkpoint_parallel_partitions = self.vertin_get_data_checkpoint_partitions(self.bit16_groups_flat[i], i)
            self.vertin_parallel_partitioned_bit16_groups.append(checkpoint_parallel_partitions)

            # verify that data partition start locations are 4-byte aligned
            for partitioned_data in data_parallel_partitions:
                assert (partitioned_data.data_ptr() % (2 * self.nccl_start_alignment_factor) == 0)
            # like above
            for partitioned_data in checkpoint_parallel_partitions:
                assert (partitioned_data.data_ptr() % (2 * self.nccl_start_alignment_factor) == 0)
            ...
            #vertin
            if not fp16_master_weights_and_gradients:
                checkpoint_partition = self.vertin_parallel_partitioned_bit16_groups[i][partition_id].to(
                    self.device).clone().float().detach()
            else:
                checkpoint_partition = self.vertin_parallel_partitioned_bit16_groups[i][partition_id].to(
                    self.device).clone().half().detach()
            ...
            checkpoint_partition_shared = checkpoint_partition.cpu().share_memory_()
            self.vertin_single_partition_of_fp32_groups.append(checkpoint_partition_shared)
            ...
            # self.single_partition_of_fp32_groups[i] ==> tensor this rank
            # replace 将 optimizer.param_groups[i]['params'] 替换为 self.single_partition_of_fp32_groups[i] <++>从原来的全局到现在的 local rank
            param_group['params'] = [self.single_partition_of_fp32_groups[i]]

            # 构建了 remote checkpoint
            self.vertin_single_partition_of_fp32_groups[
                i].requires_grad = True  # keep this in case internal optimizer uses it
            
            # vertin import
            # all param ['params'][0] = local rank checkpoint ['params'][1] = remote checkpoint
            # 也许构建两个vertin_param_group更佳？

            local_vertin_param_group = self.single_partition_of_fp32_groups[i].detach().clone().cpu().share_memory_()
            self.vertin_param_group[i]['params'] = [local_vertin_param_group]
            self.vertin_param_group[i]['params'].append(self.vertin_single_partition_of_fp32_groups[i])
            # 同理替换了原有的self.vertin_optimizer.param_groups
            # self.vertin_optimizer.param_groups[i]['params'] = [self.vertin_single_partition_of_fp32_groups[i]]
            ...
            if rank == 0:
                ...
                checkpoint_id = 2
            checkpoint_in_partition,_,_ = self.get_partition_info(
                self.round_robin_bit16_groups[i], partition_size, checkpoint_id)

            self.vertin_params_in_partition_checkpoint.append(checkpoint_in_partition)
        ...
        # vertin
        self.vertin_param_group_create_grad_shard_memory(self.vertin_param_group)
        ...
        self.vertin_is_param_in_current_partiiton_remote_checkpoint ={}
        ...
        #vertin
        for param_group in self.vertin_params_in_partition_checkpoint:
            for param in param_group:
                self.vertin_is_param_in_current_partiiton_remote_checkpoint[self.get_param_id(param)] = True
        ...
        # vertin 
        self.vertin_checkpoint_group = self.vertin_create_checkpoint_group(dist.get_rank())
        self.vertin_checkpoint_offset = []
        self.vertin_numel = []
        ...
        self.vertin_gather_bucket = []
        self.vertin_gather_remote_param = []
    ...
    def allreduce_and_copy_with_multiple_ranks(self,
        ...
        for buf, synced, bucket_rank in zip(small_bucket, self.unflatten(allreduced, small_bucket), bucket_ranks):
            if dist.get_rank(group=process_group) == bucket_rank:
                #  小模型来看并不会出现大量运行这段代码，不清楚大模型如何
                buf.copy_(synced)
        # start_event = torch.cuda.Event(enable_timing=True)
        # end_event = torch.cuda.Event(enable_timing=True)
        # checkgroup = self.create_chkprocess_groups(dist.get_rank())
        # print_rank_msg(f"{buf.size()}")
        # print_rank_msg(f"{buf}")
        # tensor_list = [torch.zeros_like(buf) for _ in range(2)]
        # # start_event.record()
        # dist.all_gather(tensor_list, buf, group=self.vertin_checkpoint_group)
        # # dist.all_reduce(buf,group=checkgroup)
        # # end_event.record()
        # # end_event.synchronize()
        # # timeoverhead = start_event.elapsed_time(end_event)
        # # print_rank_msg(f"{timeoverhead}")
        # print_rank_msg(f"{tensor_list}")
    ...
    def copy_grads_in_partition(self, param):
        ...
        grad_reduc = self.get_gradient_for_reduction(param)
        vertin_grad = grad_reduc.clone().detach()
        ...
        #vertin
        # self.vertin_checkpoint_offset.append(self.grads_in_partition_offset)
        # self.vertin_numel.append(param.numel)
        self.vertin_gather_bucket.append(vertin_grad)
        self.grads_in_partition_offset += param.numel()
    
    def vertin_allgather_bucket(self, bucket, process_group=None)
        tensor = self.flatten(bucket)
        process_group = self.vertin_allgather_bucket if process_group is None else process_group
        
        tensor_to_allgather = tensor

        tensor_list = [torch.zeros_like(tensor_to_allgather) for _ in range(2)]
        dist.all_gather(tensor_list, tensor_to_allgather,process_group)

        # tensor_list[0] low rank flatten tensor
        # tensor_list[1] high rank flatten tensor

         # 判断哪个张量是与原 tensor 不同的
        different_tensor = None
        for t in tensor_list:
            if not torch.equal(t, tensor):
                different_tensor = t
                break
                
        return tensor_list, different_tensor


    def vertin_average_tensor(self, small_bucket, process_group = None):
        process_group = self.vertin_checkpoint_group if process_group is None else process_group
        allgather_list, different_tensor = self.vertin_allgather_bucket(small_bucket, process_group=process_group)
        for buf, synced in zip(small_bucket, self.unflatten(different_tensor,small_bucket))
            buf.copy_(synced)

    def vertin_grad_gather(self, numel_per_bucket = 500000000):
        # average_tensor
        # 在每一次_copy grad 后进入，目的是为了将梯度从 GPU -> GPU‘ 
        # local grad --> self.grads_in_partition 
        # offset     --> self.vertin_checkpoint_offset[]
        # numel      --> self.vertin_numel[]
        small_bucket = []
        numel = 0
        for grad in self.vertin_gather_bucket:
            tensor = grad
            small_bucket.append(tensor)
            numel = numel + tensor.numel()
            if numel > numel_per_bucket:
                self.vertin_average_tensor(small_bucket, process_group=self.vertin_checkpoint_group)
                small_bucket = []
                numel = 0
    ...
    def reduce_ipg_grads(self):
        if self.contiguous_gradients:
            if self.extra_large_param_to_reduce is not None:
                ...
                # self.ipg_buffer[i] is a contiguous buffer of all gradients in the bucket
            ...
            for _, param, param_id in self.params_in_ipg_bucket:
                ...
                if self.partition_gradients:
                    if not self.is_param_in_current_partition[param_id]:
                        ...
                        if self.vertin_is_param_in_current_partiiton_remote_checkpoint[param_id]:
                            self.vertin_gather_remote_param.append(param)
    ...
    def get_data_parallel_partitions(self, tensor, group_id):
        ...
        for id in range(dp):
            ...
            # 假设不能完全整除- 前 remaining id rank 每个多取 1 位以保证完全切分
            if id < remaining:
    ...
    def vertin_get_data_checkpoint_partitions(self, tensor, group_id):
        partitions = []
        dp = dist.get_world_size(group=self.real_dp_process_group[group_id])
        # dp_id = dist.get_rank(group=self.real_dp_process_group[group_id])
        total_num_elements = tensor.numel()
        ...
        #     start = start + partition_size
        for id in range(dp):
            partitions_size = base_size
            if id % 2 == 0:
                start = (id + 1) * partitions_size
            if id % 2 == 1:
                partitions_size = base_size
                start = (id - 1) * partitions_size
            partitions.append(tensor.narrow(0, start, partitions_size))
        return partitions

    def vertin_param_group_create_grad_shard_memory(self, optimizer_list):
        for i, param_group in enumerate(optimizer_list):
            ...
            for k, param in enumerate(param_group['params']):
                ...
                self.vertin_param_group[i]['params'][k].grad = single_grad_partition
                print_rank_msg(f" In function {optimizer_list}")
    
    def vertin_create_checkpoint_group(self, rank):
        if rank % 2 == 0:  # rank 0 和 1 成为一组
    ...
    def backward(self, loss, retain_graph=False):
        ...
        # 构建 zoetic buffer
        self._vertin_create_buf(self.zoetic_flag, int(self.reduce_bucket_size))
